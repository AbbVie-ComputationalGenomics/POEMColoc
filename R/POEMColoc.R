...
#' snp is ignored as it is determined by pos.
#' @param gds_file
#' path to a gds file for the reference panel.  Can be NULL if specifying R2 and MAF.
#' @param subset
...
#' @param R2
#' A matrix of R2 LD between SNP positions.  This should contain LD information for top SNPs in rows and LD for positions to be imputed in columns.  Can be NULL if specifying gds_file.
...
POEMColoc <- function(dataset, dataset_list, gds_file = NULL, R2 = NULL, MAF=NULL, subset=NULL, window_size=NULL, min_MAF = 0, p1 = 10^-4, p2 =10^-4, p12 = 10^-5, dosage_method = 'dosage_alt') {
  ...
  POEMColoc_from_config(dataset_list, config, gds_file = gds_file, subset = subset,
                        ...
                        get_dosage_fn = dosage_fn, flexible_input=FALSE)
}
